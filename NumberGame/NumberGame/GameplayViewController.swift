...
class GameplayViewController: UIViewController {
    ...
    @objc private func submitButtonTapped() {
        guard gameStateManager.canSubmitInput() else {
            showAlert(title: "Invalid Input", message: "Please enter exactly \(gameStateManager.selectedGameMode.rawValue) digits!")
            return
        }
        
        switch gameStateManager.gameState {
        case .settingSecret:
            gameClient.setSecret(gameStateManager.currentInput, for: gameStateManager.currentRoomCode)
            gameStateManager.clearInput()
    ...
    private func updateUI() {
        // Simpler room display like NPC version shows "VS NPC_NAME"
        roomCodeLabel.text = "🏠 Room \(gameStateManager.currentRoomCode)"
        turnLabel.text = gameStateManager.currentPlayerTurn.isEmpty ? "Waiting..." : "\(gameStateManager.currentPlayerTurn)'s Turn"
        updateInputDisplay()
        updateSubmitButton()
    ...
    private func updateInputDisplay() {
        ...
        case .settingSecret:
            ...
            statusText = "🔢 Enter your \(gameStateManager.selectedGameMode.rawValue)-digit secret"
    ...
    private func updateSubmitButton() {
        ...
        switch gameStateManager.gameState {
        case .settingSecret:
            buttonText = "🔐 Set Secret"
        case .activeGame:
            buttonText = gameStateManager.isPlayerTurn() ? "🎯 Guess" : "⏳ Wait"
        default:
            buttonText = "⏳ Wait"
        }
    ...
    private func updateNumberPadVisibility() {
        ...
        UIView.animate(withDuration: 0.3) {
            ...
            self.submitButton.alpha = shouldShow ? 1.0 : 0.5
        }
    ...
    private func makeGuess(targetPlayer: String) {
        gameClient.makeGuess(gameStateManager.currentInput, targetPlayer: targetPlayer, roomCode: gameStateManager.currentRoomCode)
        gameStateManager.clearInput()
        updateInputDisplay()
    }
}
...
extension GameplayViewController: GameStateManagerDelegate {
    ...
    private func createPlayerCard(for player: RoomState.Player) -> UIView {
        ...
        nameLabel.text = isCurrentPlayer ? "👤 \(player.name)" : "\(player.avatar) \(player.name)"
...
extension GameplayViewController: GameClientDelegate {
    ...
    func gameClient(_ client: GameClient, didReceiveRoomList rooms: [AvailableRoom]) {
        // Room list not needed in gameplay
    }
